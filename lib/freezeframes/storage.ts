...
const ENTRY_ID_KEY = "freezeframes_lb_entry_id";

function getTodayKey(): string {
  return `freezeframes_played_${getDateString()}`;
}
...
export function saveDailyPlayed(score: number, max: number): void {
  ...
  recordDailyCompletion("freezeframes", playDate);
}

export async function fetchTodayLeaderboard(): Promise<LeaderboardEntry[]> {
  try {
    const res = await fetch("/api/freezeframes/leaderboard/daily");
    if (!res.ok) return [];
...
export async function saveLeaderboardEntry(
  ...
  try {
    const res = await fetch("/api/freezeframes/leaderboard", {
      method: "POST",
