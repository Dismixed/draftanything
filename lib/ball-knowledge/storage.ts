...
const ENTRY_ID_KEY = "bk_lb_entry_id";

function getTodayKey(): string {
  return `bk_played_${getDateString()}`;
}
...
export function saveDailyPlayed(score: number, category: string): void {
  if (typeof window === "undefined") return;
  const playDate = getDateString();
  localStorage.setItem(
    getTodayKey(),
    JSON.stringify({ score, category, ts: Date.now() }),
  );
  recordDailyCompletion("ball-knowledge", playDate);
}

export async function fetchTodayLeaderboard(): Promise<LeaderboardEntry[]> {
  try {
    const res = await fetch("/api/ball-knowledge/leaderboard/daily");
    if (!res.ok) return [];
...
export async function saveLeaderboardEntry(
  ...
  try {
    const res = await fetch("/api/ball-knowledge/leaderboard", {
      method: "POST",
